...
class Trainer:
	...
	def __init__(self, state_dim, action_dim, gridDimensions):
		...
		self.actor = model.Actor(self.state_dim, self.action_dim, gridDimensions).cuda()
		self.actor_optimizer = torch.optim.Adam(self.actor.parameters(),LEARNING_RATE)

		self.critic = model.Critic(self.state_dim).cuda()
		...
		self.action = None
		self.zero = torch.zeros(2, 2).cuda()
